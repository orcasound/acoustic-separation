...
from zeroshot.utils import get_segment_bgn_end_samples, np_to_pytorch, get_mix_data, evaluate_sdr, wiener
from zeroshot.losses import get_loss_func
...
class ZeroShotASP(pl.LightningModule):
    ...
    def __init__(self, channels, config, at_model, dataset):
        ...
        self.opt_thres = pickle.load(open('zeroshot/opt_thres.pkl', 'rb'))
...
class SeparatorModel(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.output_wav:
            ...
            # sf.write(os.path.join(self.wave_output_path, filename), batch[0], self.config.sample_rate)
            for i, dickey in enumerate(self.target_keys):
                # filename = str(batch_idx) + "_" + dickey + "_original.wav"
                # sf.write(os.path.join(self.wave_output_path, filename), batch[i + 1], self.config.sample_rate)
                # filename = str(batch_idx) + "_" + dickey + "_pred_(" + str(sdr[dickey]) + ").wav"
                # sf.write(os.path.join(self.wave_output_path, filename), preds[dickey], self.config.sample_rate)
                self.output_dict[dickey] = preds[dickey]
